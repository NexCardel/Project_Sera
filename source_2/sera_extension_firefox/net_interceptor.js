...
(function() {
    ...
    function buildSimpleParserEvidence(url, record, portal) {
        ...
        const decodeKnownField = (key, raw) => {
            ...
            const known = keyLower.includes("email") || keyLower.includes("mobile") || keyLower.includes("phone") || ["fullname", "firstname", "middlename", "lastname", "legalname", "tradename"].includes(keyLower);
        ...
        const nameKeys = new Set(["legalname", "tradename", "authsignatory", "nameasperbank", "fullname", "firstname", "middlename", "midname", "lastname", "surnameororgname", "assesseeName".toLowerCase()]);
        ...
        for (const entry of values) {
            ...
            if (nameKeys.has(key) && text && !/^(N\/A|NA|NONE|NULL)$/i.test(text)) {
                const role = key === "legalname" ? "legal_name" : (key === "tradename" ? "business_name" : "client_name");
