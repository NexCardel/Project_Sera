...
def patch_itr_protocol():
    ...
    new_func = '''    async function _handleLoginLogout(url) {
      ...
      if (!pan) {
         const loginInput = document.querySelector('input#loginUsername');
         if (loginInput && loginInput.value && /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/i.test(loginInput.value)) {
           pan = loginInput.value.trim().toUpperCase();
         }
      }
...
'''
    ...
    # We must also sync it to the other directories!
    paths_to_sync = [
