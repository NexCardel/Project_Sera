...
### P3-0 — review fixes — 2026-09-24
- Model: Gemini 3.8 Flash (review by Claude Opus 5.5)   Commit: uncommitted
- Fixed from the review:
  1. **Tests (b), (c), and (d) shared baseline (Blocking 1):** Inserting the baseline client independently on each node gave them different gids under P3-2 triggers, preventing conflict detection and field merge. Fixed: the client is created on node 0 only, synced to node 1 via `run_until_quiet()`, verified to exist on node 1 (`assert row1 is not None`), and only then partitioned. (Tests fail at setup until P3-5 exists, as intended).
  2. **Test (b) clock control (Should-fix 1):** In test (b), node 1's edit is guaranteed to have a strictly higher HLC physical time by monkeypatching `time.time` during node 1's write to be +1000s after node 0's write.
  3. **Test (e) real mid-batch crash injection (Blocking 2):** Replacing `n1._handle_session` after server startup never reached the running server socket. Fixed: an engine-agnostic `BEFORE INSERT ON clients` SQLite trigger (`inject_crash_mid_batch`) is installed on node 1 that executes `SELECT RAISE(ABORT, 'injected crash in middle of applying batch')` once 3 rows of the batch exist. Asserts exactly 0 batch rows land on node 1 (testing transaction rollback atomicity), drops the trigger, resyncs, and asserts all 5 rows land and digests converge.
  4. **Member propagation error handling & deviation (Should-fix 2):** `_propagate_all_members` now raises `RuntimeError` immediately if reading `_sync_members` from the admin DB or writing to any joiner fails (no swallowed warnings). Noted in docstring and deviations that member rows are copied directly from the admin DB across local databases in the harness rather than verified via `store_record`.
  5. **Digest error handling on FK lookups & missing row keys (Should-fix 3):** All FK lookup queries in `_compute_digest_from_conns` now raise `RuntimeError` on failure rather than swallowing with `pass`. Missing or NULL row-key columns raise `RuntimeError` (with fallback to `id` for pre-P3-2 schemas where `gid` has not yet been added).
  6. **Digest delimiter collision (Should-fix 4):** `_encode_value` now backslash-escapes `:` (the composite row-key delimiter) in addition to `|`, `,`, `=`, and `\`.
  7. **Dead code removed (Should-fix 5):** Unused `_gid_map` helper inside `_compute_digest_from_conns` removed.
  8. **Out-of-scope files reverted (Blocking 3):** Uncommitted modifications to `database.py` and `tests/test_raw_payload_db_and_srpf.py` that were mixed in from P3-1 work have been reverted via git restore, keeping P3-0 changes isolated.
