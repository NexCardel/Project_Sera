...
| P3-7a | 3 | Peer digest exchange for the shadow convergence check | T2 | P3-7 | T3 |
| P3-7b | 3 | Turn on shadow mode: panel action, common start point, re-enable guard | **T3** | P3-7 | T3 |
| P3-8a | 3 | Shadow-week readiness: parked-change age, clock-ahead warning, seal timing | T2 | P3-8 | – |
| P3-8b | 3 | Panel defect fixes (Run-now gate, conflict selection, stamps, late emit) | T2 | P3-8 | – |
| P3-9 | 3 | Go live (install converged replica, switch mode) | T2 | P3-7a, P3-7b, P3-8a, P3-8b; P3-7 criteria met | Owner decision |
...
#### P3-7a Peer digest exchange for the convergence check (T2, review T3)
Added in v1.8 (from the P3-7 review, §7). Today nothing collects peer replica digests, so the "replica digests equal" go-live criterion can't be measured.
- **Frames** (in `sync_engine.py`, inside the existing mTLS session, members only):
  - `digest_request {}` and `digest_reply {vectors, digest}`.
  - Both PCs advertise `digest: true` in HELLO. The exchange runs only when both did, so a peer on the P3-5/P3-7 build never sees an unknown frame. Check that `_check_hello` ignores the extra key on the old build; if it doesn't, stop and ask.
- **When:** in mode `shadow` only, after the change exchange and before BYE, and only when the session left both PCs' vectors equal (for both streams). At most once per peer every 10 minutes.
- **Consistency:** each side reads its replica's `_sync_vector` and computes `digest_of_replica` in **one read transaction** on the replica files. Compare only when the vectors in the reply equal the requester's own replica vectors read in the same way. Otherwise log "skipped: vectors moved", never a mismatch.
- **Result:** feed the pair into `sync_shadow.convergence_check` (or `run_shadow_checks(peer_digests=...)`) and log it to `logs/sync_shadow.log`: peer name, OK/mismatch, seconds since this PC's last local seal. Never log row contents.
- Computing a digest must not block the session past its timeouts. If it can, move it to a worker and reply with `busy`, then skip.
- **Accept:**
...
#### P3-7b Turn on shadow mode (T3, review T3)
Added in v1.8. No UI can call `sync_shadow.enable_shadow_mode` today (§9 P3-7 note 6), and the re-enable guard can be bypassed through mode `off` (§7).
- **Start-point problem (stop and ask before coding):** in mode `off` nothing is captured (`sync_capture.ACTIVE_MODES`), and each PC's replica starts as a copy of its **own** live DB. Every edit made on any PC between its P2 join/rejoin and turning shadow on is therefore never replicated, and the replicas can never match. Pick one of these with the owner, then implement it:
  1. **Admin PC's state is the start point (recommended):** the admin PC turns shadow on first. Every other PC turns it on by downloading the admin PC's snapshot (`sync_snapshot`, P2-6) as its replica **and** baseline (the baseline must also be the admin PC's snapshot, and the capture check replays only this PC's own changes from then on). Edits that exist only on a non-admin PC's live DB are lost from the replicas; the dialog says so and shows a salvage-style count first.
  2. **Procedure only:** all PCs rejoin (P2-8), nobody works, all PCs turn shadow on within the same quiet window. Simpler, but a missed edit breaks convergence for good.
- **UI:** Sera Sync panel → "Start shadow mode" (admin mode / PIN). Confirmation text explains the 7-day shadow week (P3-7 criteria).
- **Guard:** refuse when `shadow/replica_master.db` already exists or when the live `_sync_vector` has any non-own origin, not only when the mode is `shadow`. Old shadow files are renamed with the §0 rule 3 `*.bak-<ts>` pattern only by an explicit, admin-only "Reset shadow mode", which also resets the week.
- **Accept:**
  - harness: 3 nodes turn shadow on by the chosen method, edit on each, sync → replica digests equal;
  - mode set to `off` and then back on → refused;
  - an edit made on a non-admin PC before turning shadow on → handled as the chosen method says (option 1: listed in the dialog's count, not in the replica).

#### P3-8a Shadow-week readiness (T2)
Added in v1.8 (from the P3-7 and P3-8 reviews, §7). The go-live criteria and phase checks need these.
- **Parked-change age:** in mode `shadow`, remote changes are applied to the **replica** files, so parked changes are in the replicas' `_sync_parked`. `sync_panel.parked_count` on the live DBs always shows 0 in shadow mode. Read parked rows from the replicas in shadow mode and from the live DBs in live mode.
  - Panel: parked count plus the age of the oldest (`first_at`); an amber warning above 1 hour in shadow mode (go-live criterion) and a red warning above 7 days (P3-4).
  - Log the parked count and oldest age with every shadow check in `logs/sync_shadow.log`.
- **Clock-ahead warning:** handle the engine's `clock_ahead` event in `main.py` `on_sync_engine_event` (only `synced` is handled today). Show "PC <name>'s clock is <n> minutes ahead" in the panel, in the same banner as "needs updating", and add an activity-log line (phase check 18).
- **Seal timing:** log the time the seal listener (seal + `mirror_own_changes_to_replica`) takes on the committing thread to `logs/sync_shadow.log`, at most one line per minute (count, mean and max ms), so the shadow week measures it. Don't move the work to another thread in this WP; report the numbers.
- **Accept:** a parked change in the replica shows in the panel with its age; a `clock_ahead` event shows the banner; the timing line is written; no row contents in any log.

#### P3-8b Panel defect fixes (T2)
Added in v1.8 (from the P3-8 review, §7).
- "Run now" is enabled only in mode `shadow` (the 30-min timer already checks this).
- The conflicts table tracks the selected conflict by `(db, id)` and re-selects it after the 3-second refresh. "Keep" acts on that id, never on a row position.
- "Last Successful Sync" shows the engine's stamps and the address book's stamps in one format.
- The "Run now" worker doesn't emit on a dialog that has been closed (guard the emit, or run the check through `SeraApp`).
- **Accept:** one UI test per defect.
...
5. **Release 3.1:** turn on shadow mode on all PCs (P3-7b: Sera Sync panel → Start shadow mode, the way P3-7b's chosen start point says). Watch the panel for 7 days (P3-7 criteria).
...
| **Open, no WP (P3-7 review, 2026-09-25): the shadow convergence check can't compare PCs.** `sync_shadow.convergence_check` takes already-collected peer replica digests, but nothing collects them: the P3-5 session protocol has no frame for it. The 30-min timer and "Run now" (P3-8) run only the capture check. The go-live criterion "replica digests equal within 2 minutes of quiet" can't be measured, so P3-9 can't start. | **Must be done before P3-9.** Now WP **P3-7a** (v1.8): a `digest_request` / `digest_reply` exchange in `sync_engine.py` (mTLS, members only), run after a session leaves both vectors equal, feeding `run_shadow_checks(peer_digests=...)` and logged to `logs/sync_shadow.log`. |
| **Open, no WP (P3-7 review): shadow mode can be re-enabled through mode `off`.** `enable_shadow_mode` refuses only while the mode is `shadow`. If the mode is set to `off` and shadow is enabled again, the new replica is built from the live DBs, whose `_sync_vector` already counts remote changes as received that only the old replica holds. No peer resends them, so the new replica permanently disagrees with the other PCs, and P3-9 would install it. No UI can do this today. | Now part of WP **P3-7b** (v1.8), together with the missing "turn on shadow mode" action and its start point: refuse when `shadow/replica_master.db` already exists (or when the live `_sync_vector` has any non-own origin), not only when the mode is `shadow`. The risk ends at P3-9 (shadow is never re-enabled after go-live). |
| **Open, owner decision (P3-7 review): every office-mode PC serves full snapshots to any member at any time.** Since P3-7, the permanent 49159 server routes `{"t":"snapshot"}` to `sync_snapshot.handle_snapshot_session` on every PC (`sync_office.dispatch_session`). Before, only the admin PC served snapshots, and only while its "Add workstation" pairing window was open. Callers are mTLS-authenticated members, who already receive the same data through sync, so this is a larger attack surface rather than a new leak. | Owner to decide: keep it (P4-4's "returning member downloads a snapshot" may want any PC to serve one) and record that here, or limit serving to the admin PC while a pairing window is open, plus P4-4's `NEED_SNAPSHOT` case. |
| **Open, no WP (P3-3 note, P3-7 review): the cost of sealing on the committing thread was never measured.** P3-3 left "to be measured in P3-7" for sealing on the calling thread (which can be the UI thread) in shadow/live mode. P3-7 didn't measure it, and in shadow mode each seal also runs `sync_shadow.mirror_own_changes_to_replica` (extra SQLCipher connections plus an `apply_batch` on the replica) on the same thread. | Measuring is now part of WP **P3-8a** (v1.8). The mirroring stops at P3-9 (it does nothing outside shadow mode); sealing on the committing thread continues in live mode. Measure UI-thread time per commit during the shadow week (e.g. a timing line in `logs/sync_shadow.log`) before P3-9. If it's noticeable, move the seal listener's work to a background thread. |
| **Open, no WP (P3-8 review, 2026-09-25): "Use discarded value" can never restore anything.** `sync_apply.py` writes a `_sync_conflicts` row only when the row is deleted/tombstoned ("edit discarded by delete", "edit after delete discarded"; P3-4 log note 10). `sync_panel.use_discarded_value` is a plain `UPDATE`, so it always affects 0 rows for these conflicts (since the P3-8 fixes it leaves the conflict in place and says so). The panel's "use discarded value" action (§5 P3-8) therefore does nothing today. | Needs a WP (owner to assign; T3, it touches tombstones): re-create the row: remove the tombstone, re-insert with the original gid, translate wire-form FK gids in "edit after delete discarded" values and gid-form composite keys back to local ids (the `sync_apply._resolve_key` / FK translation machinery). The P3-8 review-fixes entry in §9 has the details. Until then, consider hiding the button for `sync_panel.DELETED_ROW_REASONS`. |
| **Open, spec contradiction (P3-8 review): ordinary same-field losses are not in the conflicts list.** The D7 settings row above says "the losing value shows in the conflicts list (P3-8)", and §5 P3-8 describes the list as field conflicts. But P3-4 writes no conflict row for an ordinary LWW loss (P3-4 log note 10: without version vectors a late older edit can't be told from a concurrent one). Two PCs changing the same setting or client field: the losing value is dropped with no record. | Owner to decide: accept it and correct the D7 row above, or give P3-4 a way to record LWW losses (e.g. log a conflict only when the losing edit's HLC is within N minutes of the winner's, or add per-column version vectors, T3). |
| **Open, no WP (P3-8 review): the clock-drift panel warning doesn't exist.** The "Clock far off on one PC" row above cites a "panel warning (P3-3)". The engine emits a `clock_ahead` event (`sync_engine.py`, from `sync_apply`'s `clock_ahead` parking), but `main.py`'s `on_sync_engine_event` handles only `synced`, and no dialog shows it. A PC with a far-ahead clock gets its changes parked silently. | Now part of WP **P3-8a** (v1.8): handle `clock_ahead` in `on_sync_engine_event` and show "PC <name>'s clock is <n> minutes ahead" in the Sera Sync panel (same banner as "needs updating"), plus an activity-log line. |
| **Open, no WP (P3-8 review): parked-change age is not shown anywhere.** P3-4 says changes parked for over 7 days "surface in the panel", and the P3-7 go-live criteria include "no parked changes older than 1 hour". The panel shows only a total count (`sync_panel.parked_count`), and nothing logs `_sync_parked.first_at`. The go-live criterion can't be checked. | **Must be done before P3-9.** Now part of WP **P3-8a** (v1.8), which also reads the replicas' `_sync_parked` in shadow mode (the live DBs' count is always 0 there): show the oldest parked change's age (and a warning past 7 days) in the panel, and log the oldest age with each shadow check in `logs/sync_shadow.log`. |
| **Open, no WP (P3-8 review): small panel defects.** (a) "Run now" calls `run_shadow_checks` in any mode, while the 30-min timer runs only in `shadow`: in mode `off` it logs a "no baseline" failure, and after P3-9 it would log false capture-check mismatches. (b) The conflicts table is rebuilt every 3 s and the selection is by row position, so "Keep" can delete a different conflict if a new one arrives between selecting and clicking. (c) The "Last Successful Sync" column mixes the engine's local-time stamps with the address book's stored format. (d) The "Run now" thread emits `shadow_check_finished_signal` on the dialog even if it was closed meanwhile. | Now WP **P3-8b** (v1.8): enable "Run now" only in mode `shadow`; track the selected conflict by `(db, id)` and re-select it after refresh; format both stamps the same way; guard the emit (or run the check through `SeraApp`, not the dialog). |
...
- Model: Gemini 3.8 Flash (review by Claude Opus 5.5)   Commit: 067828b
...
### P3-3 — Capture triggers, sealer, HLC — In review — 2026-09-25
- Model: Claude Opus 5.5   Commit: uncommitted
- Tests: new `tests/test_sync_capture.py`, 74 passed. Accept: `test_public_write_method_produces_expected_changes` (parameterised over 49 `SeraDatabase` write methods in `WRITE_CASES`, each with required and forbidden `(table, op)` sets; `test_every_public_write_method_is_listed` scans `database.py` with `ast` and fails if a public method containing INSERT/UPDATE/DELETE/REPLACE is neither in `WRITE_CASES` nor in `NOT_CALLED` with a reason — reviewer: check that list), `test_external_raw_write_is_captured_by_timer_seal`, `test_no_pending_rows_left_after_seal` (plus a pending-count check in every parameterised case), `test_hlc_monotonic_under_stepped_back_clock`. Also: tick/observe rules, drift guard (`ClockAhead` past +1 h, exactly +1 h accepted), counter overflow, mode off captures nothing, `applying=1` suppresses capture, first seal sends all columns then only changed ones, unchanged rewrite → no change, one HLC per row + contiguous `origin_seq` + own `_sync_vector`, new device_id → new stream from seq 1, `update_client` resubmit is not a delete, cleared field → delete + tombstone + clock cleared and re-create resends all columns, composite keys as gids + cascade children skipped, row-key change → delete of the old key, `cell_formatting` `"services"` literal vs numeric key, FK columns hold gids (NULL for an unknown referent), cross-DB `tracker_dump.client_id` → client gid, admin changes signed/verified (tampered or wrong key fails), non-admin staff change not sealed + logged, nested connections, sealer failure never breaks the write, triggers present per replicated table and none on local tables, no PySide6.
  Full suite: 1353 passed / 16 failed / 3 skipped — the 10 documented pre-existing failures + the 6 P3-0 convergence tests (they need P3-4/P3-5). The `next_seq_stream` fix (deviation 7) was made after that run; `test_sync_capture`, `test_sync_tables`, `test_sync_schema` rerun after it: 110 passed.
- Files: new `sync_capture.py`, new `tests/test_sync_capture.py`; `database.py` (`_connect`/`_connect_raw` mark and seal after the outermost commit; `seal_pending`, `get_sync_mode`/`set_sync_mode`, `start_seal_timer`/`stop_seal_timer`, `_admin_signer`; `_sync_mode` read in `_init_schema`); `sync_tables.py` (`ensure_sync_infrastructure` also creates the capture triggers); `main.py` (starts the 5 s seal timer after the DB opens, stops it on quit).
- Deviations from spec (for the T3 review):
  1. **Triggers also check the mode.** Their `WHEN` is `applying IS NOT 1 AND _sync_meta.mode IN ('shadow','live')`. The spec's pseudocode only checks `applying`; the sealer only runs in shadow/live, so with the spec's trigger every PC in mode `off` (all of them today) would fill `_sync_pending` forever.
  2. **Net effect per row, not per trigger row.** The spec says pending rows are grouped by `(tbl, rid)`, with deletes by OLD key. `update_client()` does `DELETE FROM client_values/client_services WHERE client_id=?` and re-inserts everything on every save. Sealed literally, every save would tombstone every value, and P3-4's "tombstone exists → drop the upsert" would lose them on every other PC. So a pending delete becomes a delete only if no row with that key exists at seal time; otherwise the current row is sealed as an upsert (and only changed columns go out). Upserts read the row as it is now. Items are emitted in order of their first pending id (parents before children).
  3. **Extra trigger `_sync_ak_<t>`:** an UPDATE that changes the row key itself (e.g. `client_values.client_id` moved to another client, or a gid rewrite outside apply) also records a delete of the OLD key. Otherwise the old key would live on at other PCs.
  4. **Clock presence row `col='*'`** in `_sync_clock` for every sealed row: marks "sealed before" (the spec's first-seal rule) and gives `client_services` (no data columns) a row-level clock for P3-4's `set` mode.
  5. **Delete clears the row's clock rows; re-creating a tombstoned key locally removes the local tombstone** and sends all columns.
  6. **Admin-scoped change without the admin key:** the pending row is dropped (not kept), a warning is logged, and no clock row is written. Keeping it would retry forever.
  7. **`_sync_meta.next_seq_stream`** (new key): `next_seq` belongs to that stream. If `device_id`/`stream_id` changes (rejoin), the new stream starts at 1. Without this it would continue the old number and P3-4 could never advance its vector contiguously.
  8. **HLC counter overflow:** past `9999` in one millisecond the logical time moves to the next ms (keeps string order). An HLC for master.db and rawPayload.db from one process never goes backwards between the two DBs (a per-process max per device, in addition to each DB's `last_hlc`).
  9. **Own stream in `_sync_vector`** = `next_seq - 1` after each seal, so HELLO (P3-5) shows what this PC has of its own stream.
  10. **Seal after commit only at the outermost `_connect`/`_connect_raw` of a thread** (nested connections exist in `database.py`; an inner seal would wait on the outer connection's write lock). Master is sealed before raw. It uses a 2 s busy timeout; on failure it logs and leaves the rows for the 5 s timer. The write itself is never affected.
- Notes for later WPs:
  - Public API (`sync_capture`): `ensure_capture_triggers(conn, db_name)`, `trigger_names(t)`, `HLC` (`parse`, ordering), `tick(last, device_id, now_ms=None)`, `observe(last, remote, device_id, now_ms=None)` (raises `ClockAhead(remote_hlc, ahead_ms)`; the caller parks with reason `clock_ahead` and doesn't move the clock), `seal(db_path, hex_key, db_name, *, master_path=None, get_signer=None, now_ms=None, timeout=5.0) -> SealResult(changes, pending, skipped_admin, tables)`, `sign_change(change, private_key)`, `verify_change(change, admin_pubkey)`, `change_body`, `canonical_json`, `key_text`, `vhash`, `SealTimer`, `ACTIVE_MODES`, `PRESENCE_COL`, `MAX_CLOCK_AHEAD_MS`. `SeraDatabase`: `seal_pending(which=("master","raw"))`, `get_sync_mode()`, `set_sync_mode(mode)` (writes both DBs; P3-7/P3-9 use it), `start_seal_timer()`/`stop_seal_timer()`.
  - **Formats (P3-4 must read the same):** `row_key` is always a compact JSON array (`["<gid>"]`, `["<key>"]`, `["<client gid>","<column gid>"]`; `cell_formatting` keeps the literal `"services"`). `data` is a JSON object of changed columns only, never `id`/`gid`/row-key columns, FK columns as gids (NULL when unknown), bytes as `{"$b64": ...}`. The signature covers `canonical_json({origin, origin_seq, hlc, tbl, row_key, op, data})` with `data` parsed. `op` is `upsert` or `delete`; a delete has `data` NULL. `origin` is the stream id (`<device>:m`/`:r`).
  - **P3-4, apply needs `applying=1` in the same transaction:** the triggers read `_sync_flags` inside the writer's transaction, so the apply engine's own writes aren't captured. When applying a change, P3-4 must also write `_sync_clock` rows with the same `vhash` function, computed on the value in its gid form (as in `data`), not on the translated local id. Otherwise the next local seal of that row resends the remote values as a local edit. Also write the `'*'` presence row.
  - **Spec issue for the owner (P3-4, not changed here):** P3-4 says "if a tombstone exists for the row, drop the upsert". That's right for gid rows (a re-created row gets a new gid). For **natural/composite keys** it's wrong: clearing a client field deletes the `client_values` row (tombstone for `[client gid, column gid]`). If anyone fills that field again later, the upsert for the same key would be dropped on every other PC, for ever. The same goes for `app_settings` keys that are deleted and re-set, `cell_formatting` (`clear_cell_formatting` then colour again), and `client_services` (P3-4's `set` mode, row-level LWW, may already cover it). This PC seals the re-creation as a normal upsert with a newer HLC than the tombstone, so P3-4 can compare `(hlc, origin)` against the tombstone for non-gid keys. **Stop and ask the owner before P3-4 implements the tombstone rule.**
  - **P3-6 / owner:** `assign_or_get_alias()` (called at start-up on every PC to bind the PC's station alias to a staff slot) writes `staff_users`, which is `admin_lww`. On a non-admin PC that write isn't sealed, so the PC's alias binding stays local and the capture check (P3-7) will see the difference. Either `staff_users.alias` is really per-PC (then it belongs in a local table) or it's roster data (then only the admin PC may set it). Needs a decision.
  - **P3-6:** start-up maintenance (the dataset_key recompute in `_init_raw_schema`, `re_resolve_all_tracker_dumps`, `resequence_client_serial_numbers`) now produces changes on every PC in shadow/live mode whenever it changes values. The recompute rewrites every `tracker_dump.dataset_key` at every start; unchanged values seal to nothing (vhash), but the rows are still read once per start.
  - Triggers are `CREATE TRIGGER IF NOT EXISTS`: if a later WP changes the trigger SQL, it must `DROP TRIGGER` the old ones first (names from `trigger_names`).
  - `client_container_notes` still doesn't exist (P3-1); `ensure_capture_triggers` skips registered tables that don't exist, so creating that table later and reopening the DB gives it triggers automatically. Until then, `save_srpf_container_media` notes stay local (test `save_srpf_container_media` expects no changes).
  - Rows that already existed when a PC goes to shadow mode have no clock rows, so their first local edit sends all their columns with a new HLC. That can overwrite a concurrent edit of another column on another PC (spec behaviour, "first seal → all columns"). P3-7 starts every PC from the same snapshot, which limits this to that first edit.
  - Not exercised: DOM_Parser/SDC_Parser processes writing while the app runs (covered only by a raw `sqlcipher3` write in the test), two real PCs.

### P3-3 — review fixes — 2026-09-25
- Model: Claude Opus 5.5   Commit: uncommitted
- The review (Claude Opus 5.5) found nothing blocking. It agreed with all 10 deviations above and with the two owner questions. Fixed from its "should fix" list:
  1. **Connection count leak:** `_enter_connection()` now runs after `sqlite3.connect()` succeeds, in both `_connect` and `_connect_raw`. Before this, a failed connect left the thread's count raised for good, and its writes were only sealed by the 5 s timer. Test: `test_failed_connect_does_not_leak_connection_depth`.
  2. **rawPayload.db mode follows master.db:** at start-up `_init_raw_schema` sets rawPayload.db's `_sync_meta.mode` to master.db's if they differ (for example after a P0-8 auto-heal created a fresh rawPayload.db in mode `off`). Test: `test_raw_payload_mode_follows_master_at_startup`.
  3. **Write-method list:** the scanner now also counts methods that call a writing method (transitively), not only methods with SQL in their body. New cases: `save_scc_settings`, `tag_client_scc_verified`, `get_srpf_containers` (a read method that rebuilds containers when there are none). `upgrade_all_placeholder_client_names` and `load_ini_defaults` now set up data and check for a real change. `run_startup_maintenance` and `restore_from` are in `NOT_CALLED` with reasons. Found on the way (not fixed, outside P3-3): `_clean_ligature_noise_from_names` selects `tracker_dump.client_name`, which doesn't exist, so its transaction always rolls back and it never cleans anything.
  4. **Signature domain separation (new deviation 11):** admin-scoped changes are signed over `CHANGE_SIG_DOMAIN` (`b"sera-sync-v3-change\x00"`) followed by `canonical_json(change_body)`. P2-2 records are signed over `canonical_json(record)` with no prefix, so a signature made for one format can never verify as the other, even though one admin key signs both. The change format is new in P3-3; §5 P3-3 says only "sign the change (`sig`) with the admin key", and P2-2's `staff_change` record type isn't used for this. `sync_admin`'s `staff_change` record (whose `_check_shape` accepts any fields) is still there. P3-4 should remove it or give it a field check. Test: `test_change_signature_is_domain_separated_from_admin_records`.
  5. **Shorter lock hold:** `seal()` works through `_sync_pending` in transactions of at most `SEAL_BATCH_ROWS = 500` rows, so a big start-up batch (such as the dataset_key recompute touching every tracker_dump row) no longer holds the write lock for the whole seal. The net-effect rule (deviation 2) still holds across batch boundaries, because each batch compares against the rows as they are at that moment. The after-commit seal on the caller's thread (which can be the UI thread) now waits at most 0.25 s for the lock, not 2 s; on timeout it leaves the rows to the timer. Test: `test_seal_in_small_batches_keeps_net_effect_and_numbering`.
  6. **No device id:** `set_sync_mode("shadow"|"live")` now refuses (ValueError) when either database has no `device_id`, so pending rows can't pile up without anyone sealing them. Test: `test_set_sync_mode_refuses_without_device_id`.
- **Open, needs an owner decision (before P3-7 enables shadow mode, and before P3-9 because `restore_from` can still be used in live mode until P4-3b): sequence numbers can repeat when the database file is replaced.** `next_seq` lives inside master.db / rawPayload.db. If the file is replaced, the counter goes back:
  - `restore_from` of an older backup of this PC's own database: numbering continues from the backup's value, reusing numbers already sent.
  - a legacy staged swap (P0-4) of another PC's database while in shadow mode: the new-stream fallback (deviation 7) restarts at 1.
  Other PCs already hold changes with those `(origin, origin_seq)` pairs, so P3-4 skips the new changes as "already applied", silently. Options: (a) a high-water mark per stream kept outside the database (e.g. in `keys/` or `office.json`, DPAPI not needed, just monotonic), with `next_seq = max(db value, high-water mark) + …`; (b) an epoch in the stream id (`<device_id>:m:<epoch>`), bumped whenever the DB file is replaced or restored, so a replaced file always starts a fresh stream. (b) also needs P3-4/P3-5 to treat older epochs of the same device as finished streams. Not implemented; P3-3's code works as documented until the owner picks one.
- **Performance, to be measured in P3-7:** in shadow/live mode every committed write seals on the calling thread (at most one 0.25 s lock wait, then the work of the seal itself). Timer seals are now bounded to 500 pending rows per transaction.
- Tests: `tests/test_sync_capture.py` 82 passed (8 new). Full suite: 1362 passed / 16 failed / 3 skipped (the 10 documented pre-existing failures + the 6 P3-0 convergence tests); nothing new.

### P3-3 — owner decision: sequence high-water mark (option A) — 2026-09-25
- Model: Claude Opus 5.5   Commit: uncommitted
- **Owner decision 2026-09-25:** option A from the review-fixes entry above. The highest sequence number issued per stream is kept outside the database, so replacing the DB file can't make this PC reuse numbers.
- What changed (`sync_capture.py`, `database.py`):
  - New file `keys/sync_seq.json` next to master.db: `{"<device_id>:m": n, "<device_id>:r": n}`. Plain JSON, not secret. Written with `sera_keys.atomic_write`, only ever raised (`record_seq_mark`), never lowered or deleted.
  - Each seal transaction numbers from `max(next_seq from the DB, mark + 1)`. That covers `restore_from` of an older own backup, a staged swap, and a new stream after a rejoin (deviation 7). `seal(..., seq_state=path)`; `SeraDatabase.seal_pending` passes `sync_capture.seq_state_path(db_path)`. `SealResult` gained `last_seq`.
  - The mark is raised **after** each batch commits, never before. A mark written for a batch that then rolled back would leave a gap in the stream, and a gap stalls every other PC's vector for that stream (P3-4 advances only while contiguous).
  - An unreadable or malformed `sync_seq.json` raises `SeqStateError`: sealing stops and pending rows are kept, rather than numbering without the mark. The user's write still succeeds (the after-commit seal logs it; the timer retries). A missing file counts as 0 (a fresh PC).
- Tests: 4 new in `tests/test_sync_capture.py` (86 total, all pass): `test_replaced_db_file_never_reuses_sequence_numbers` (copies the DB files, seals 3 more changes, puts the older files back, reopens: numbering continues after the mark with no gap), `test_new_stream_after_rejoin_respects_the_mark`, `test_seq_mark_only_goes_up`, `test_unreadable_seq_mark_stops_sealing_and_keeps_pending`. Full suite: 1366 passed / 16 failed / 3 skipped (the 10 documented pre-existing failures + the 6 P3-0 convergence tests); nothing new.
- Notes for later WPs:
  - **Remaining gap after a restore (P3-5 / P4-3b):** the mark prevents *reuse*, not *loss*. If this PC's DB is replaced with an older copy, the changes numbered between the copy's `next_seq` and the mark are gone from this PC. Any PC that received them keeps them and forwards them (A→B→C, §4.4). A PC that never received them waits for numbers nobody holds, so its vector for this stream stops advancing. P3-5 must detect a stream that can't advance (e.g. no member offers the next seq) and fall back to `NEED_SNAPSHOT` or a documented "skip to" record. P4-3b (restore → the state on every PC) should raise the mark and announce the new state rather than rely on it.
  - A crash in the gap between a batch's COMMIT and the mark write leaves the mark behind the DB. That only matters if the DB file is *also* later replaced by a copy older than that batch; then those few numbers could be reused. Accepted as very unlikely; closing it fully would need the mark written first, which creates the gap problem above.
  - Don't delete or hand-edit `keys/sync_seq.json`. §0 rule 3 applies: when a PC is rebuilt, keep the file or copy it along with `keys/`. Losing it doesn't break anything by itself; it just removes the protection until the next seal writes it again.

### P3-4 — Apply engine — In review — 2026-09-25
- Model: Claude Opus 5.5   Commit: uncommitted
- Owner decisions taken at the start of this WP (2026-09-25, via AskUserQuestion):
  1. **Tombstones for natural/composite keys: newer re-create wins** (the spec issue P3-3 raised). Rows keyed by gid: delete always wins, as §5 says. client_values, client_services, cell_formatting, app_settings, sdc_session_timelines: an upsert newer than the tombstone brings the row back; an older one is dropped.
  2. **The client merge on the internal PK (owner decision 2026-09-24) is built in P3-4**, not in a separate WP.
  3. **Accept tests (b)–(e):** the P3-0 tests need P3-5 sessions and still fail. The same scenarios run in `tests/test_sync_apply.py` with changes handed straight from one node's `_sync_changes` to the other's apply. The P3-0 tests are unchanged.
- Tests: new `tests/test_sync_apply.py`, 55 tests, all pass. P3-0 scenarios: `test_p30_b_same_field_higher_hlc_wins`, `test_p30_c_different_fields_both_survive`, `test_p30_d_delete_wins_over_concurrent_edit_with_conflict_row`, `test_p30_e_crash_mid_batch_leaves_nothing_and_resync_completes`, plus A→B→C forwarding. One test per §5 bullet: `applying` flag reset after success and error, nothing captured, sort by hlc, idempotent, admin signature (valid / tampered / missing / no admin key), missing parent parked then applied, later change waits behind a parked one, edit-before-create parked as `incomplete_row`, parked list after 7 days (no values), cross-DB tracker waits for its client, per-column LWW, FK and `cell_formatting` "services"/numeric keys, append once, gid tombstone, natural-key re-create / concurrent clear vs newer edit / newer delete vs older edit / partial-row losing delete, set add/remove, app_settings re-set, natural-key merge of services and staff, rename onto an existing name, `_sync_changes` recorded unchanged, contiguous vector, observe, clock-ahead parking, malformed batch rejected, pending local edits sealed first. Merges: same PAN on two PCs → one client (children, tracker rows, token, audit row), edit to a merged-away client redirected, archived not merged, three-way merge in any order, merge racing a delete, a merge decision reaching a PC that never saw both rows, the rename race, `emit=False`, sequence numbers of emitted changes, the raw re-point job surviving a crash. `test_random_operations_and_partial_syncs_converge` (3 seeds in the suite; 130 more seeds run by hand in a scratch script: all converged after the fixes below).
  Full suite: 1421 passed / 16 failed / 3 skipped — the 10 documented pre-existing failures + the 6 P3-0 convergence tests (they need P3-5). After that run, `apply_changes` was added to `NOT_CALLED` in `tests/test_sync_capture.py` (P3-3's write-method scanner; it writes with `applying=1` by design). Sync test files rerun after it: all pass.
- Files: new `sync_apply.py`, new `tests/test_sync_apply.py`; `database.py` (`apply_changes`, `_office_admin_pubkey`); `sync_capture.py` (`note_observed`); `tests/sync_harness.py` (digest, deviation 6); `tests/test_sync_capture.py` (`NOT_CALLED` entry).
- Deviations from spec (for the T3 review):
  1. **Tombstone rule for natural/composite keys** as the owner decided (above). A delete older than the row's newest clock resets only the columns older than the delete to their "absent" value (DEFAULT, else NULL, else ''/0) and keeps the row; a resurrecting upsert fills the columns it doesn't carry the same way, stamped with the tombstone's clock. Both orders give the same row. `set` mode (client_services) follows the same rule at row level.
  2. **Merge decisions are replicated as changes** (new op `merge`: `{tbl, row_key:[loser], op:"merge", data:{"into": winner}}`, in the deciding PC's own stream, numbered like the sealer, seq mark raised after commit). §5 says the natural-key merge "is deterministic, so all PCs converge". Stress testing showed it isn't: whether two rows collide depends on what a PC holds at that moment (e.g. PC A renames service "P" to "Q" while PC B creates "P": B sees A's "P" before the rename and merges, A never does). The same holds for the PAN merge (PC A archives a client, PC B sees it still active halfway through a batch). With the decision sent as a change, every PC performs the same merge whatever it holds. A copy not here yet is covered by the alias. A deleted copy deletes the merged row, the same as a delete arriving after the merge. **P3-5 must treat `merge` like any other change.**
  3. **PAN merge is checked once at the end of each batch**, not after each change, so a state the batch only passes through doesn't cause a merge. Archived clients never take part in detection; a received `merge` is applied regardless.
  4. **In a merge the name (natural key) merges by LWW** like other columns, so a later rename of either copy wins on every PC. Only `clients.client_id_token` is kept from the winner (D8), and redirected changes don't carry it.
  5. **References to a deleted row become NULL** on the receiving PC (audit_log.client_id/service_id; tracker_dump / sdc_session_timelines through `run_raw_repoints`), the same as a PC that never had the row stores (P3-1 note: referent unknown → NULL).
  6. **Harness digest (`tests/sync_harness.py`, a P3-0 file):** an FK id whose row no longer exists is hashed as NULL instead of the raw local id. P3-7 says local ids are excluded, and the sealer already sends such a reference as NULL. Without this, digests depended on whether two PCs happened to hand out the same local ids.
  7. **Bad admin signature:** the change is dropped, logged and **not recorded** in `_sync_changes`, so a forged change can't occupy `(origin, origin_seq)` ahead of the genuine one. Its stream's vector waits at that gap until the genuine change arrives.
  8. **Parked changes are recorded** in `_sync_changes` (so they're forwarded and the vector advances); the parked table is only this PC's to-do list. "Any later change for the same row" means a later **HLC**; parked changes are retried in HLC order. Reasons: `missing_parent`, `incomplete_row` (an edit for a row whose create hasn't arrived; NOT NULL columns missing), `behind_parked`, `clock_ahead`.
  9. **Local pending rows are sealed inside the apply transaction** before anything is applied, so an incoming edit can't undo a local delete the timer hadn't sealed yet.
  10. **Conflict rows** are written for gid rows in both directions: "edit after delete discarded" (receiver of an edit for a tombstoned gid) and "edit discarded by delete" (receiver of a delete that removes newer local edits). No conflict rows for ordinary LWW losses: without version vectors a late older edit can't be told from a concurrent one (P3-8 may revisit).
- Notes for later WPs:
  - Public API (`sync_apply`, no PySide6): `apply_batch(db_path, hex_key, db_name, changes, *, master_path=None, admin_pubkey=None, get_signer=None, seq_state=None, now_ms=None, timeout=5.0, emit=True) -> ApplyResult` (applied, skipped, dropped, rejected, ignored, parked, unparked, conflicts, sealed, raw_repoints, emitted, merges, clock_ahead, tables), `retry_parked(...)`, `run_raw_repoints(master_path, raw_path, hex_key)`, `parked_changes(db_path, hex_key, older_than_seconds=None, now=None)` (no values), `PARK_SURFACE_SECONDS`, `MERGE_ACTION`, `ApplyError`. `SeraDatabase.apply_changes(which, changes, admin_pubkey=None, now_ms=None)` applies, runs raw re-points and retries rawPayload.db's parked changes after master.db. It reads `admin_pubkey` from office.json when not given.
  - **P3-5:** send changes ordered by hlc, `data` as stored. Apply master.db batches before rawPayload.db ones where possible (raw rows wait for their client otherwise). `ApplyError` means a malformed batch: drop the session, don't retry it blindly. `ClockAhead` shows up as `ApplyResult.clock_ahead` [(origin, ms)] for the "PC <name>'s clock is ahead" message. `ApplyResult.emitted > 0` means this PC added merge changes to its own stream: poke peers. A stream whose vector waits at a gap (rejected forged change, or the restore case in the P3-3 notes) needs the P3-5 "can't advance" handling.
  - **P3-7:** apply to shadow replicas with `emit=False` (they share the live DB's stream id; emitting there would reuse sequence numbers) and let merge decisions come from the live DB's detection, or give replicas their own stream. `run_raw_repoints` takes explicit paths, so it works for replicas.
  - **P3-6 / owner:** `delete_client()` writes an audit row with the deleted client's local id and leaves tracker rows pointing at it on the PC that deleted it. Other PCs now store NULL there (deviation 5), and the digest treats both the same (deviation 6), but the data still differs until `delete_client` nulls or removes those references itself. Also decide what tracker rows of a deleted client should show (they now become client_id NULL on other PCs).
  - Staff (admin_lww) merges on a PC without the admin key are applied locally but not sent (they can't be signed). Only a double-admin rename race could make PCs differ; the digest check (P3-7) would show it.
  - `sync_admin`'s unused `staff_change` record type is still there (P3-3 review suggestion); P3-4 doesn't use it (changes carry their own signatures).
  - Not exercised: two real PCs, P3-5 sessions (the P3-0 harness tests (a)–(f) still fail until then), DOM/SDC parser processes writing during an apply.

### P3-4 — review fixes — 2026-09-25
- Model: Claude Opus 5.5 (review by Claude Opus 5.5: nothing blocking, `--reviewed-by` recorded)   Commit: uncommitted
- Fixed from the review's "should fix" list:
  1. **Append tables are insert-only:** a remote `delete` for an `append` table (audit_log) is ignored and logged. Nothing in the app deletes audit rows (checked: no `DELETE FROM audit_log`), so origin and receivers still agree. A remote `merge` is accepted only for `MERGEABLE_TABLES` (clients, services, staff_users); any other table → ignored and logged.
  2. **Behind-parked check follows `_sync_alias`** (`_park_key`): a later change sent under a merged-away gid now waits behind a parked change of the winner. Parked rows are keyed this way when loaded, when parked and when retried.
  3. Tracker note no longer repeats its date.
  4. Test fix: `test_edit_to_merged_away_client_is_redirected` depended on which random gid sorted first. When the losing copy was node 0's, node C pulled both copies, merged them before its edit, and so edited nothing (about half the runs). C now pulls only the holder of the losing copy. It passed 8 runs in a row.
- **Waiting for the owner:** review item 4, the harness digest change (deviation 6 in the entry above) edits a P3-0 acceptance file and needs the owner's explicit OK under §0 rule 4.
...
### P3-6 — Make start-up maintenance admin-only / id-independent (F12, F13) — Done — 2026-09-25
- Model: Gemini 3.8 Flash (review by Claude Opus 5.5: `--reviewed-by` recorded)   Commit: 27bfc64
...
### P3-5 — Session protocol, forwarding, poke, scheduler — Done — 2026-09-25
- Model: Claude Opus 5.5   Commit: uncommitted
- Owner decisions taken at the start (2026-09-25, via AskUserQuestion):
  1. **Build and test only, no app wiring.** `main.py` is untouched; every PC is in mode `off` today, so nothing would sync anyway. P3-7 wires the engine in when it turns shadow mode on (see the notes below).
  2. **A stuck stream is detected and shown, nothing is skipped.** The P3-3 note asked P3-5 to "fall back to NEED_SNAPSHOT or a skip-to record" for a gap nobody holds; neither is done here. The real fix stays with P4-3b / P4-4.
- Tests: new `tests/test_sync_engine.py`, 29 tests, all pass. Accept: P3-0 **(a)** `test_convergence_random_operations_and_partitions` and **(f)** `test_convergence_abc_forwarding_never_connected` (unchanged P3-0 tests, now driven by real sessions; (b)–(e) pass too); `test_session_interrupted_mid_transfer_resumes_next_round` (1200 changes, session cut after the first batch: rows and vector match, the next round finishes with no duplicates and equal digests); `test_poke_path_syncs_within_3_seconds` (schedulers on 1-hour rounds, so only the poke can do it). Also: schema mismatch ends the session and both sides say who needs updating; wrong office tag; non-members not dialled; a new member record reaches a PC that never talked to the admin, and TLS trust is rebuilt; a revoke reaches the others and the removed PC is refused; Hand over admin tells the named PC (`admin_named_here`); a forged member record is ignored; peer vectors and the working address are recorded; HELLO gossips addresses; batches ≤ 500 changes and ~1 MB (a single bigger change still goes alone); only what the peer lacks is sent; a malformed batch drops the session and the receiver backs off; a change sent under the other DB is refused; an apply failure rolls back and the next round retries; NEED_SNAPSHOT below a (patched) compaction floor; mode off exchanges members only; shadow mode accepts nothing without a replica and uses one when given; one session per peer; 3 in total; crossed dials (smaller device id keeps its session); scheduler round; poke format/validation/debounce; DiscoveryService passes pokes on; seal listener; stuck stream reported once and not skipped; no PySide6. `tests/test_sync_transport.py::test_file_streams_in_chunks` also checks the new `recv_file(on_progress=)`.
  The convergence/engine/harness/maintenance files passed 3 runs in a row (51 passed, 1 skipped each).
  Full suite: 1467 passed / 11 failed / 3 skipped. The 11 are the documented pre-existing failures (the same set P3-6 reported: dom_page_replace, gst_dom_tracker, purge_duplicates, raw_payload_db_and_srpf, updater, vsdc_beeper, vsdc_gemini_enricher x5). The 6 P3-0 convergence failures are gone. That run started before two small final fixes (pokes sent from their own thread; the harness waits for just-closed sessions to drain before its quiet check), so all `tests/test_sync_*.py` files were re-run on the final code: 554 passed, 1 skipped (P3-0 (g), P4-4).
- Files: new `sync_engine.py`, new `tests/test_sync_engine.py`; `tests/sync_harness.py` (engine per node, mode live, `run_until_quiet` drives rounds, `sync_with`, `run_sync_round`); `database.py` (`set_seal_listener`, called from `seal_pending` when a seal produced changes); `sync_discovery.py` (`DiscoveryService(on_poke=)`: pokes share the beacon port); `sync_tables.py` (index `ix_sync_changes_hlc`); `sync_transport.py` (`Session.recv_file(..., on_progress=)`); `sync_snapshot.py` (`download_snapshot` uses `recv_file`, P2-6 note); `tests/test_sync_transport.py` (progress assert).
- Deviations from spec (for the T3 review):
  1. **HELLO has two extra fields:** `mode` and `accepts`. A PC that can't apply remote changes (mode off, or shadow without a P3-7 replica) says `accepts: false` and nobody sends it changes. `members_rev` is a 16-hex digest over the verified member + office_admin records, not a number: records have their own revs, and a sum could be equal for different lists.
  2. **`CHANGES.stream` names the DB file** (`"master"` / `"raw"`), not an origin stream. A batch mixes origins in `(hlc, origin, origin_seq)` order, so parents from another PC usually come before their children. Every item's origin must end in `:m` / `:r` to match, otherwise `bye bad_batch`. master.db batches go before rawPayload.db ones (P3-4 note).
  3. **Frames beyond the spec's list:** `done` (end of one side's changes), `need_snapshot {streams}` sent in place of changes, BYE carries the final vectors (step 4 stores them), and `bye` with a reason for every early end (`schema`, `proto`, `office`, `identity`, `busy`, `bad_batch`, `apply_failed`, `protocol`). The schema BYE carries `mine`/`yours` as in §5; the event says `who` needs updating (the lower version).
  4. **The sender walks a cursor** after the peer's HELLO vectors instead of re-reading them from each ACK, so a change the receiver can't take (a forged admin change it rejects, P3-4 deviation 7) is sent once per session, not in a loop.
  5. **Where remote changes go:** mode `live` → `SeraDatabase.apply_changes`; mode `shadow` → the `shadow_apply` hook (P3-7), without it nothing is accepted; mode `off` → sessions still run HELLO and the membership exchange (so membership also spreads between members: this closes the P2-2 review's "plan gap" once the engine runs).
  6. **Session limits:** a per-peer lock plus 3 slots, both directions. When two PCs dial each other at once, the one with the smaller device id keeps its outgoing session (the other side waits up to 5 s for it). A refused responder reads the peer's HELLO before `bye busy`, because on Windows closing with it unread resets the connection and the `bye` is lost.
  7. **Errors:** `ApplyError` (malformed) → `bye bad_batch`, and the receiver doesn't dial that peer for 5 min ("don't retry blindly"). Any other apply error (DB locked, the P3-0 (e) injected crash) → `bye apply_failed`, retried next round. The batch rolled back with its vector, so nothing partial remains.
  8. **Compaction floor:** `SyncEngine.compaction_floors()` returns `{}` until P4-4, so NEED_SNAPSHOT is only reachable in tests. A PC told it needs a snapshot only reports it (event + log).
  9. **The responder doesn't record the peer's address:** it knows the IP but not the peer's listening port. Only the initiator updates `_local_addresses` (`record_successful_session`).
  10. **Pokes** are sent by a small thread (the seal can run on the UI thread), at most every 0.2 s, to each member's first connect-order address, UDP port 49156. In the app they arrive on the beacon socket, so `DiscoveryService` passes datagrams containing `"poke"` to `on_poke` (`SyncEngine.handle_poke` validates magic, office tag and membership). Pokes aren't authenticated (the office tag is in every beacon); a forged one only makes this PC open a mutual-TLS session with a real member, at most once per second per member.
  11. **Harness (P3-0 file, needs the owner's OK under §0 rule 4):** `run_until_quiet` used to only wait for open sessions, and `_handle_session` was a placeholder loop "until the P3-5 engine is wired in". Now every node runs a `SyncEngine` in mode `live` (the harness replicates into the live DBs; shadow replicas are P3-7), and `run_until_quiet` runs sessions between all connected pairs until a round moves nothing, raising `TimeoutError` otherwise. The 20 s scheduler is too slow for the tests' 5 s timeouts. **No test in `test_sync_convergence.py` was changed.**
  12. **Index `ix_sync_changes_hlc`** on `_sync_changes(hlc, origin, origin_seq)` (in `sync_tables.ensure_sync_tables`), so each batch is read by index instead of sorting the whole change log.
  13. `download_snapshot` now uses `Session.recv_file`; its three chunk errors ("Expected chunk frame", "More file data than announced", "Empty chunk") are now one `SnapshotError("Bad file transfer for <file>: …")` with the transport's message.
- Notes for later WPs:
  - Public API (`sync_engine`, no PySide6): `SyncEngine(db, transport, *, app_dir=None, office_tag=None, admin_pubkey=None, connect=None, beacon_sighting=None, poke_port_for=None, poke_listen=None, on_event=None, shadow_apply=None, own_cert_pem=None, interval=20, jitter=5, poke_debounce=1)` with `.handle_session(session)` (server handler), `.sync_with(device_id) -> SessionResult`, `.sync_all()`, `.start()` / `.stop()`, `.handle_poke(data, ip)`, `.notify_local_change()`, `.peer_status()`, `.stalled_streams()`, `.own_vectors()`, `.reachable_members()`, `.compaction_floors()`. `SessionResult(device_id, initiator, sent, received, applied, emitted, members_exchanged, members_changed, need_snapshot, tables, peer_vectors, error)`. Also `make_poke`, `parse_poke`, `stream_db`, `PROTO`, `SessionAborted`.
  - Events (`on_event(kind, info)`): `synced` (sent, received, applied, tables), `sync_failed` (reason), `needs_update` (name, mine, yours, who), `need_snapshot`, `members_changed`, `admin_named_here`, `clock_ahead` (name, ahead_ms), `parked`, `stalled` (name, waiting_for, have_up_to).
  - **P3-7 (wiring, must do):** in office mode build `SyncTransport(load_cert_chain_args, MemberSet.from_db(...))`, `engine = SyncEngine(db, transport, beacon_sighting=discovery.get_beacon_sighting, on_event=..., shadow_apply=<replica apply with emit=False>)`, `transport.serve(<handler>, port=49159)`, `DiscoveryService(on_poke=engine.handle_poke)` (or set `discovery.on_poke`), `db.set_seal_listener(engine.notify_local_change)`, `engine.start()`, and `engine.stop()` on quit. **Port clash:** `sync_office.AddWorkstationSession` binds its own snapshot server on 49159 (`SO_EXCLUSIVEADDRUSE`), which fails once the engine's server holds the port. The permanent server's handler must route a first frame `{"t":"snapshot"}` to `sync_snapshot.handle_snapshot_session` and everything else to the engine, and Add workstation must use that server (and rebuild its `MemberSet`). `handle_session` reads the first frame itself today, so that dispatch needs a small wrapper that peeks at the first frame.
...
- Fixed blocking #1 ("The capture check fails on any PC that has received a change from
  ...
  replaying every `_sync_changes` row (own **and** forwarded) held data the live DB never had.
  `capture_check` now replays only this device's own-origin changes
  (`_own_captured_changes`, replacing `_all_captured_changes`), matching the spec's literal
  wording ("baseline snapshot + this PC's own changes"). New regression test
  `test_capture_check_still_passes_after_receiving_a_remote_change`: joiner receives and
  applies a remote change, then its own capture check must still pass (it failed before the fix).
- Fixed blocking #2 ("Turning shadow mode on a second time silently loses remote data for
  good"): `enable_shadow_mode` now raises `RuntimeError` if `_sync_meta.mode` is already
  `'shadow'`, before touching any file. New regression test
  `test_enable_shadow_mode_refuses_when_already_on`.
- Not fixed, accepted as documented follow-ups (all called "not blocking" by the reviewer):
  - **#7** (rejected/parked remote changes still count as received in the live DB's
    bookkeeping): `_record_receipt_in_live` records every item `shadow_apply` was handed,
    regardless of whether the replica actually accepted it. `apply_batch`'s `ApplyResult` has no
    per-item outcome to filter on today, only aggregate counts — a correct fix needs that API
    extended first. Left as a known gap for whoever does that.
  - **#8** (the permanent server now answers `{"t":"snapshot"}` from any connected member at any
    time, not only while "Add workstation" is open): accepted for now — a session only exists
    between mTLS-verified office members in the first place (the same PCs that already receive
    this data through ordinary sync), so this widens *convenience*, not who can see the data.
    Flagged for a deliberate decision (e.g. restrict to the admin PC + open pairing window) in a
    later WP rather than guessed at here.
  - **#9** (a write landing between the snapshot and `set_sync_mode("shadow")` is in neither the
    baseline nor `_sync_changes`): narrow, accepted race; `enable_shadow_mode` doesn't hold the
    DB lock across both steps.
  - **#4** (seal-timing not measured; `mirror_own_changes_to_replica` runs synchronously on the
    committing thread, which can be the UI thread): correct and unaddressed; P3-3 already flagged
    this as "to be measured in P3-7" and it still isn't. No perf measurement or async offload
    done here.
  - **#5** (the working tree mixes in P3-8-shaped work): not from this WP — a concurrent session
    was already adding the Sync Status panel (`sera_sync_dialog.py`'s member-presence code,
    `main.py`'s refresh-by-table code, untracked `sync_panel.py` + its tests) to the same working
    tree while this WP ran; see this WP's original §9 entry's notes. Whoever commits should stage
    P3-7's files separately from that panel work so each commit matches its tracker row.
- Tests: `tests/test_sync_shadow.py` now 17 tests, all pass. Full suite re-run: 1536 passed / 11
  failed / 3 skipped (798s) — the same 11 pre-existing failures as before this WP, plus the 2 new
  regression tests.
...
### P3-8 — review fixes — 2026-09-25
- Reviewed by: Claude Opus 5.5. The previous entry's "Deviations from spec: none" was wrong -- the review found three blocking problems and this entry corrects that.
- Blockers fixed:
  1. **`use_discarded_value` deleted the conflict row even when the UPDATE changed nothing** (`sync_panel.py`). `sync_apply.py` only ever writes a conflict the instant it deletes/tombstones the row (both call sites, P3-4 log note 10), so by the time the panel shows it the row is already gone and the UPDATE always affects 0 rows -- the button destroyed the only record of the discarded edit and always reported failure. Now it deletes the conflict only when `cur.rowcount > 0`. Re-creating a deleted row (un-tombstoning, re-inserting with the gid, translating wire-form FK gids and composite natural keys back to local ids) is real new scope, not a one-line fix, and is **not done** -- "use discarded value" is effectively inert for every conflict shape sync_apply.py produces today, until a future WP builds resurrection. New `sync_panel.DELETED_ROW_REASONS` names the two reasons this covers; `test_use_discarded_value_against_a_real_delete_conflict` (new, in `tests/test_sync_panel.py`) exercises the real shape via `sync_apply.apply_batch` instead of a hand-inserted conflict row (the previous test's `reason="lww"` on a still-live row was a shape P3-4 never produces). The dialog's failure message now says "row was deleted on another PC" for these two reasons instead of the generic "may be gone".
  2. **Nothing ran the shadow checks.** `sync_shadow.run_shadow_checks`'s own docstring says "called on demand (panel) and from a periodic timer once P3-8 wires one in" -- neither existed. Added: a 30-minute `QTimer` in `main.py` (next to the P3-7 engine setup, mode `shadow` only, runs once at start-up too) that calls `sync_shadow.run_shadow_checks(db, app_dir)` in a background thread; and a "Run now" button in the panel (`sera_sync_dialog.py`) that calls the same function directly (the dialog only holds the `SyncEngine`, not `SeraApp`, so it can't reach the timer's closure) via a new `shadow_check_finished_signal` to get back onto the GUI thread. **Peer-digest collection for the convergence check is still not done** -- no wire protocol exists to ask a peer for its replica digest, and building one is out of this WP's scope (session-protocol changes belong with P3-5's owner). Only the capture check runs; `run_shadow_checks(peer_digests=None)` skips convergence, as documented in its own signature.
- Should-fix items addressed:
  4. `peer_status()[dev]["needs_update"]` (already populated by the engine itself, `sync_engine.py`'s `_needs_update`) is now read in `_refresh_members` and folded into the existing `members_network_warning` banner, worded per the P3-5 note ("PC <name> needs updating" / "This PC needs updating"). `stalled_streams()` is polled in `_refresh_sync_status` (like `peer_status()` already was) and shown as "Stalled: N stream(s)" in the summary line. `need_snapshot` and `clock_ahead` are **not** surfaced -- they weren't in the P3-5 note's explicit list for P3-8, and `need_snapshot` can't fire in production yet (`compaction_floors()` returns `{}` until P4-4, per that engine comment).
  5. `on_sync_engine_event` now skips queuing a "synced" event unless `db.get_sync_mode() == "live"`, so shadow mode (remote changes going to the replica) no longer pulses the sidebar pill or reloads windows for data that didn't change.
  6. `_refresh_members`'s `last_ok` now prefers `peer_status()[dev]["last_ok"]` over `_local_addresses.local_ok_at` when the engine has it, so a peer that always dials us doesn't show "never" next to an "Online" status.
- Minor items addressed: conflicts table shows "(deleted)" instead of the literal text "None" for `kept` (always `None` for every conflict sync_apply.py currently writes); the throttle's `threading.Timer` is now cancelled on `aboutToQuit` (new `_cancel_synced_tables_timer`).
- Not done: splitting the P3-7/P3-8 commits (both are still uncommitted in the same working tree; the owner will commit them separately per the review, once P3-7 itself is reviewed).
- Tests: 25 more tests added across `tests/test_sync_panel.py` (net +1, one test rewritten to use a real `sync_apply`-produced conflict), `tests/test_sync_engine_ui.py` (+5), `tests/test_sync_status_panel_ui.py` (+7) -- 68 P3-8 tests total, all pass. Full suite re-run after these fixes: see the next tracker update for the count.
- Notes for later WPs:
...
### P3-8a — Shadow-week readiness — In review — 2026-09-26
- Model: Gemini 3.7 Flash (reviewed by Claude Opus 5.5)   Commit: uncommitted
- Tests: 85 passed in target test suite (`tests/test_sync_panel.py`, `tests/test_sync_shadow.py`, `tests/test_sync_status_panel_ui.py`, `tests/test_sync_engine_ui.py`); 121 passed when combined with engine and digest suites (`tests/test_sync_engine.py`, `tests/test_sync_digest_exchange.py`); full suite 1567 passed / 11 failed / 3 skipped (the same 11 pre-existing failures in broad UI/parser suite: dom_page_replace, gst_dom_tracker, purge_duplicates, raw_payload_db_and_srpf, updater, vsdc_beeper, vsdc_gemini_enricher x5).
- Deviations from spec: none.
- Review fixes & non-blocking items addressed:
  1. **Coalesced clock-ahead events & rate-limited activity logging**: `sync_engine.py` coalesces/deduplicates `applied.clock_ahead` per device in `_receive_changes`, setting `result.clock_ahead_seen = True` and emitting at most one event per device per session. `main.py`'s `on_sync_engine_event` further rate-limits logging per device to at most once per hour unless the minute offset changes. Test `test_on_sync_engine_event_clock_ahead_deduplicates_repeated_events` verifies repeated events from the same device produce a single log line.
  2. **Clock-ahead warning banner 1-hour expiration**: `sync_engine.peer_status()` records `seen_at` in the peer `clock_ahead` state and clears it after 1 hour without a new clock-ahead event, rather than immediately dropping the warning on the next session. Tests: `test_peer_status_clears_clock_ahead_after_one_hour` (sets `seen_at` 70 min ago, asserts `clock_ahead=None` in returned status and in `_peers`); `test_peer_status_keeps_clock_ahead_within_one_hour` (30 s ago — not expired).
  3. **Shadow-only seal timing & flush on quit**: `main.py`'s `_seal_timing_cb` guards calls to `sync_shadow.record_seal_timing` to only record in `db.get_sync_mode() == "shadow"`. `main.py` connects `app.aboutToQuit` to `sync_shadow.flush_seal_timing` so buffered metrics write out before process exit. Tests: `test_seal_timing_cb_only_records_in_shadow_mode` (mode live/off → not called; mode shadow → called once); `test_flush_seal_timing_called_on_quit` (the quit lambda calls `flush_seal_timing` with `app_dir`).
  4. **Exact age label assertions**: `test_sync_status_panel_ui.py` asserts exact summary string `Parked: 1 (oldest: 2h)`.
- Notes for later WPs:
  - **Parked-change age (`sync_panel.py`, `ui/dialogs/sera_sync_dialog.py`, `sync_shadow.py`)**: `parked_stats` and `parked_summary` return `(count, oldest_age_seconds)` using `first_at`. In mode `shadow`, `SeraSyncDialog._refresh_sync_status()` reads parked changes from `shadow/replica_master.db` and `shadow/replica_raw.db` (falling back to live DBs in mode `live`), formatting the oldest age with `format_age()`. Displays an amber warning for $> 1$ hour in shadow mode and a red warning for $> 7$ days. `sync_shadow.run_shadow_checks()` logs `parked_changes count=X oldest_age=...` to `logs/sync_shadow.log`.
  - **Clock-ahead warning (`main.py`, `sync_engine.py`, `ui/dialogs/sera_sync_dialog.py`)**: `on_sync_engine_event` handles `kind == "clock_ahead"`, logging `"PC <name>'s clock is <n> minutes ahead"` to the activity stream (phase check 18). `sync_engine.peer_status()` records `clock_ahead` for the peer, which `SeraSyncDialog._refresh_members()` displays in the `members_network_warning` banner.
  - **Seal timing logger (`sync_shadow.py`, `database.py`, `main.py`)**: `database.py`'s `_seal_after_commit()` measures total duration on the committing thread and passes it to `sync_shadow.record_seal_timing(dur_ms, app_dir)`. `SealTimingLogger` aggregates measurements and writes at most one summary line per minute (`seal_timing count=N mean=X.Xms max=Y.Yms`) to `logs/sync_shadow.log`, with zero row contents logged.
...
### P3-7b — Turn on shadow mode: panel action, common start point, re-enable guard — Done — 2026-09-26
- Model: Claude Opus 5.5   Commit: uncommitted
- Owner decisions taken at the start (2026-09-26, via AskUserQuestion, the spec's "stop and ask"):
  1. **Start point 1a:** the admin PC turns shadow on from its own live DBs. Every other PC downloads the admin PC's **replica** and, after a restart, uses it as its live DB, replica **and** baseline.
  2. **Edits only on a non-admin PC:** the dialog shows a salvage-style count first, then after the restart Sera offers the P2-8 salvage import. Imported rows are captured as normal changes and reach every PC.
  3. **Old P3-7 test rewritten** (§0 rule 4, owner OK): `test_enable_shadow_mode_backs_up_an_existing_replica_instead_of_deleting_it` expected off → on to succeed, which the P3-7b Accept forbids. It is now `test_enable_shadow_mode_refuses_while_an_old_replica_exists`. Its §0-rule-3 check (renamed to `.bak`, never deleted) moved to the new Reset test.
  4. **A PC added mid-week:** office snapshots ("Add workstation", P2-6) are exported with mode `off`. The non-own-origin guard applies only where a replica is built from the live DBs (the admin path).
- Tests: new `tests/test_sync_shadow_start.py` (22) and `tests/test_sync_shadow_start_ui.py` (8), all pass. `tests/test_sync_shadow.py` (1 test rewritten, see above) passes.
  - Accept: `test_three_nodes_start_shadow_edit_sync_and_replicas_converge` (3 harness nodes start by 1a, edit on each, sync; replica digests equal and capture checks OK on all three); `test_mode_off_then_on_again_is_refused_on_the_admin_pc` and `..._on_a_non_admin_pc`; `test_edit_made_before_start_is_counted_and_not_in_the_replica`.
  - Also covered: the salvage import replicates to the other PCs and digests still agree; skipping salvage; admin start refused when the live vector has a remote origin; only the admin PC in mode shadow serves its replica; admin start refused on a non-admin PC; request refused on the admin PC; the Add-workstation snapshot has mode off; the swap keeps this PC's `_local_addresses` and newer `_sync_members` revs and sets its own device id; a damaged staged download leaves the live DB alone; cancel; reset renames the files and resets the week, after which a non-admin PC can start again; reset refused while a start is staged; `started.json`; no row contents in `sync_shadow.log`; no PySide6. UI: button states, admin start after PIN, PIN required, cancel, reset, and the non-admin download → count → stage → restart / decline / error paths.
  - Full suite: 1597 passed / 11 failed / 3 skipped (842 s). The 11 are the same failures every recent WP has reported (dom_page_replace, gst_dom_tracker, purge_duplicates, raw_payload_db_and_srpf, updater, vsdc_beeper, vsdc_gemini_enricher x5). None is new.
- Files: `sync_shadow.py` (guard in `enable_shadow_mode`, `_REPLICA_LOCK`, new P3-7b section); `sync_office.py` (`dispatch_session` routes `shadow_snapshot`); `sync_snapshot.py` (`_clean_exported_db` sets mode `off`); `main.py` (`_run_pending_shadow_start` before key resolution and DB open, plus a catch-up mirror thread after wiring in mode shadow); new `ui/dialogs/shadow_start_dialog.py`; `ui/dialogs/sera_sync_dialog.py` (Shadow mode row with Start/Reset, PIN re-check, download worker plus `shadow_start_downloaded_signal`, `_closed` flag via `closeEvent`/`done`); `tests/test_sync_shadow.py`; the two new test files.
- Deviations from spec (for the T3 review):
  1. **Baseline:** the spec says a non-admin PC's baseline must be the admin PC's snapshot. It is, but the PC's live DB is replaced by that same snapshot (option 1a as the owner chose it). Otherwise the capture check (baseline + own changes = live) could never pass on a PC whose live data differed.
  2. **The download is the admin PC's replica, not its live DB.** In mode shadow the admin's live `_sync_vector` counts remote changes the live data doesn't hold (`_record_receipt_in_live`). New frame `{"t":"shadow_snapshot"}` on the permanent 49159 server. It is served only by the office admin PC in mode shadow; anything else gets `{"t":"error"}`. The export runs under a new process-wide `_REPLICA_LOCK` (also taken by `shadow_apply`, own-change mirroring and reset), so the master and raw files are one consistent state. Side effect: a commit on the admin PC can wait while another PC's download is exported (seconds, once per PC).
  3. **Guard:** `enable_shadow_mode` now raises `ShadowStartRefused` (a `RuntimeError`) in three cases: mode `shadow`, any replica/baseline file exists, or the live `_sync_vector` has a non-own origin with `max_seq > 0`. It still allows mode `live`, because the harness starts nodes in live. The UI entry points (`start_shadow_mode`, `request_shadow_start`) require mode `off`. A non-admin PC isn't checked for non-own origins (decision 4). Its live DB, replica and baseline are all replaced, so the divergence the guard prevents can't happen.
  4. **Start-up swap:** before the move, the replica and baseline get this PC's device id and stream, mode `off`, and empty `_sync_peer_vectors`. The new live DB also gets this PC's `_local_*` tables and any `_sync_members` row the old DB had at a higher rev, then mode `shadow`. Local-mode tables (`client_activity_stats`, `client_recent_activity`) are **not** carried over, because they key on local client ids that differ in the new DB. The old live DBs and sidecars move to `shadow/pre-start-<ts>/`. A failure while moving puts everything back, and the start is recorded as `incoming/shadow_start.pending.json.failed` without being retried. A key that can't be loaded leaves the pending start in place for the next launch.
  5. **"Admin mode / PIN":** the Sera Sync panel is already inside admin mode. Start and Reset ask for the PIN again (`AdminPinDialog`).
  6. **`started.json`** (`shadow/`) records `started_at`, `method` (`admin` / `admin_replica`) and `source_device`. The panel shows "day N of 7" from it. Reset renames it, which is how "resets the week" is done.
  7. **Salvage wording:** P2-8's `SalvageDialog` is reused as-is. Its "Don't import" confirmation still says "legacy folder"; the old files are actually in `shadow/pre-start-<ts>/`. Cosmetic.
- Open items for the owner:
  1. **Reset on the admin PC is a dead end once it has received changes.** After Reset, the admin path is refused by the guard (the live vector has other PCs' origins), and the admin can't download from itself. A safe restart there would need its live DB's received-change bookkeeping cleared (forwarded `_sync_changes` rows and non-own `_sync_vector` rows) so peers resend. Then every other PC would also have to Reset and restart. Not built, since it isn't in the spec; the Reset dialog warns about it.
  2. The P3-7 review's #9 race (a write between the admin's snapshot and `set_sync_mode("shadow")`) is unchanged.
- Notes for later WPs:
  - `sync_shadow` P3-7b API: `start_shadow_mode(db, app_dir)`, `handle_shadow_snapshot_session(session, db, app_dir)`, `open_admin_session(engine, db, app_dir)`, `admin_device_id(db, app_dir)`, `request_shadow_start(db, app_dir, session) -> ShadowStartPlan(new_dir, source_device, vectors, report, files)`, `stage_shadow_start`, `cancel_shadow_start`, `has_pending_shadow_start`, `apply_pending_shadow_start(app_dir) -> {pre_start_dir, source_device} | None`, `pending_shadow_salvage`, `plan_shadow_salvage`, `apply_shadow_salvage(app_dir, take_legacy=)`, `skip_shadow_salvage`, `reset_shadow_mode(db, app_dir)`, `shadow_status(db, app_dir)`. Exceptions: `ShadowStartRefused`, `ShadowStartError`. Constants: `FRAME_SHADOW_SNAPSHOT`, `STARTED_FILE`, `START_DIRNAME`, `SHADOW_WEEK_DAYS`.
  - **P3-9:** `shadow/pre-start-*` folders and `salvage_pending.json.*` are left in place. The go-live swap must not treat them as replicas.
  - **P3-8b:** this WP added `done()`/`closeEvent` setting `self._closed` in `SeraSyncDialog`, and a guarded emit for the download worker. P3-8b's "Run now" emit guard can use the same flag.
  - Working tree: P3-7a, P3-8a and P3-8b are also uncommitted in this tree and share `sync_shadow.py`, `main.py` and `sera_sync_dialog.py`. Commit P3-7b hunk by hunk, as the earlier entries say.
...
### P3-7b — review fixes — 2026-09-26
- Model: Claude Opus 5.5 (fixes for the Claude Opus 5.5 review)   Commit: uncommitted
- **Blocking #1 fixed (Reset → Start left a permanent gap in a PC's own stream).** The reviewer's first option: `apply_pending_shadow_start` now calls `_carry_own_changes` for both files. It applies this PC's own changes above the downloaded replica's count for its own stream from the old live DB's `_sync_changes` into the new replica and live DB, before they are placed. They must be consecutive; otherwise the start is refused and nothing changes. The baseline stays the plain download, so the capture check (baseline + own changes = live) still holds. The admin PC and the others receive the carried changes in the next session, like any unsent change. Regression test `test_reset_then_start_keeps_own_changes_the_admin_never_received`: B seals 2 changes while partitioned from the admin, then Reset and Start. B's live DB and replica hold them, the capture check passes, a new edit on B syncs, the cluster goes quiet, and the replica digests are equal. The test was confirmed to fail with the carry-over disabled (scratch plugin, not in the repo).
- **#2 fixed:** `reset_shadow_mode` refuses on the admin PC once its live `_sync_vector` has another PC's origin ("Reset the other PCs instead"). Tests: `test_reset_refused_on_the_admin_pc_once_it_has_received_changes`, `test_reset_allowed_on_the_admin_pc_before_it_received_anything`. The Reset dialog text was updated to match.
- **#3 fixed (crash window at the swap):** before moving anything, the swap writes `incoming/shadow_start/installing.json` (the list of moves and the pre-start folder). At the next start `_recover_install` reads it:
  - if the last move is done, it finishes the install (`started.json`, `salvage_pending.json`, clean-up);
  - otherwise it moves back every move that was done, newest first, marks the start failed, and says the database was not changed, which is then true.
  - Tests: `test_crash_after_the_files_moved_is_finished_at_the_next_start`, `test_crash_halfway_through_the_moves_is_put_back_at_the_next_start` (crash simulated with a `BaseException` and `_undo_on_error=False`).
- **#5 fixed:** a download (and a staged start) must contain both `master.db` and `rawPayload.db`. Test: `test_download_without_raw_is_refused`.
- **#6 fixed:** `request_shadow_start` checks that `session.peer_device_id` is the verified office admin before sending anything. Test: `test_request_refused_when_the_session_is_not_to_the_admin_pc`.
- **#4 (for P3-9, not changed here):** `sync_snapshot._clean_exported_db` always exports mode `off`. That's right during the shadow week, but after go-live a PC added through "Add workstation" would start in `off` and capture nothing. **P3-9 must make the exported mode follow the office's mode (live → live).**
- **#7, #8 not changed (recorded):**
  - #7: while the admin PC exports its replica for a download, its own-change mirroring waits on `_REPLICA_LOCK`. Only members can trigger this, and it happens once per PC start.
  - #8: P0-4's legacy `apply_pending_swap` can still replace a shadow-mode live DB until P3-9 disables legacy pushes.
- Tests: `tests/test_sync_shadow_start.py` 28, `tests/test_sync_shadow_start_ui.py` 8 and `tests/test_sync_shadow.py`, 56 in total, all pass. Full suite: 1611 passed / 11 failed / 3 skipped (959 s), the same 11 failures that were there before; none is new.

### P3-8b — Panel defect fixes — Done — 2026-09-26
- Model: Claude Sonnet 5   Commit: uncommitted at the time this entry was first written; the
  dialog fixes (and three of the six original tests) were then swept into commit `1943770`
  under the P3-8a label, since all of P3-7a/P3-7b/P3-8a/P3-8b shared the same uncommitted
  working tree at that point (see the "Working tree" notes on the entries above). This entry's
  own commit is therefore `1943770` plus whatever commit later holds the remaining tests and
  this doc update -- not "uncommitted" as first written here.
- Fixed all 4 defects listed in §5/§7 (P3-8 review, "small panel defects"), all in `ui/dialogs/sera_sync_dialog.py`:
  1. **(a) "Run now" gated to mode shadow.** `_on_run_shadow_check` now no-ops (doesn't call `sync_shadow.run_shadow_checks`) unless `self.db.get_sync_mode() == "shadow"`, matching the 30-minute timer's own check in `main.py`'s `_run_shadow_check_bg`. The button itself is kept enabled/disabled in step with the mode from `_refresh_sync_status` (a new `self._shadow_check_running` flag stops that from re-enabling it while a check is already in flight; `_on_shadow_check_finished` clears the flag and recomputes the enabled state instead of always setting `True`).
  2. **(b) Conflict selection tracked by `(db, id)`.** `_refresh_sync_status` now reads `_selected_conflict()` before rebuilding `conflicts_table`, and re-selects the row whose cached `(db, id)` matches after the rebuild -- previously a plain row-index restore (there wasn't one at all: the table was always rebuilt with no selection preserved) meant "Keep" could act on whatever conflict happened to land in that row position once a new one arrived and shifted the sort order.
  3. **(c) Stamps use one format.** The engine's `peer_status()[...]['last_ok']` (epoch seconds) now formats as `strftime("%Y-%m-%dT%H:%M:%SZ")` in UTC, the same format `sync_discovery`'s address-book `local_ok_at` already uses, instead of `datetime.fromtimestamp(...).isoformat(timespec="seconds")` (naive local time, no suffix -- also timezone-dependent, so it was a latent date-boundary flake on top of the format mismatch). **Owner note (Claude Opus 5.5 review):** this makes the whole column UTC, so an office on e.g. +05:30 sees this stamp 5.5 hours off from other times shown in the dialog. Meets the spec's wording ("format both stamps the same way") but the owner may prefer both stamps rendered in local time instead -- flagged, not changed, since the spec didn't pick one.
  4. **(d) Late emit guarded.** The `_on_run_shadow_check` worker's `finally` block now checks `getattr(self, "_closed", False)` (the flag P3-7b's `closeEvent`/`done()` already set) before emitting `shadow_check_finished_signal`, so a check still running when the dialog closes doesn't touch a dead dialog's widgets.
- **Review fixes (Claude Opus 5.5, same day):** reviewed the code above (already committed by then) plus the still-uncommitted tests, found no blocking problem, and fixed 4 non-blocking findings:
  1. **Selection-gap in (b).** If the previously-selected conflict is no longer in the list (resolved elsewhere, or pushed past `list_conflicts`'s `LIMIT 200`), Qt kept the old row index selected, which then silently pointed at whatever conflict now sat there -- a second quick "Keep" could dismiss the wrong one. `_refresh_sync_status` now calls `conflicts_table.clearSelection()` when there *was* a previous selection but no row matches its `(db, id)` after the rebuild. New test `test_conflict_selection_cleared_when_the_selected_conflict_is_gone`.
  2. **Selection test didn't actually prove `at`-based sorting.** `test_conflict_selection_survives_refresh_by_db_and_id_not_row_position`'s two conflicts had `at` values in the same order as their ids, so it passed equally if the table sorted by id -- it wasn't testing what its comment claimed. Rewritten so the first-inserted (lowest id) conflict has the *newest* `at` and must display first; the test now asserts that ordering explicitly before exercising selection tracking.
  3. **Timing-dependent test.** `test_run_shadow_check_worker_does_not_emit_after_dialog_closed` raced a 0.2s worker sleep against a 0.4s wait, which could flake on a slow machine. Rewritten with a `threading.Event` the worker blocks on until the test has set `_closed = True` and released it, removing the race; the post-release wait is now a generous fixed pump instead of a timing bet.
  4. Reviewed and accepted as-is: (c)'s UTC-vs-local tradeoff above (owner decision, not a defect).
- Tests: `tests/test_sync_status_panel_ui.py`, one new test per original defect plus the review's new selection-gap test, plus one updated pre-existing test: `test_run_shadow_check_calls_run_shadow_checks_and_refreshes` (now starts shadow mode first, since the bug it exercised -- running in any mode -- is what fix (a) removes), `test_run_shadow_check_button_disabled_outside_shadow_mode`, `test_run_shadow_check_is_a_noop_outside_shadow_mode`, `test_conflict_selection_survives_refresh_by_db_and_id_not_row_position`, `test_conflict_selection_cleared_when_the_selected_conflict_is_gone`, `test_last_successful_sync_stamps_use_the_same_format_for_engine_and_address_book`, `test_run_shadow_check_worker_does_not_emit_after_dialog_closed`. `tests/test_sync_status_panel_ui.py`: 28 passed. Full suite (before the review fixes): 1616 passed / 11 failed / 3 skipped (909s) -- the same 11 failures every recent WP has reported (dom_page_replace, gst_dom_tracker, purge_duplicates, raw_payload_db_and_srpf, updater, vsdc_beeper, vsdc_gemini_enricher x5); none new. Re-run after the review fixes: 1617 passed / 11 failed / 3 skipped (893s), same 11 pre-existing failures, none new (one more test than before: the new selection-gap test).
- Deviations from spec: none. The spec's item (a) says "enabled only in mode shadow"; implemented as both an enabled-state sync in `_refresh_sync_status` and a no-op guard in the click handler itself (belt-and-suspenders against a stale click queued the instant the mode changes), which doesn't change the observable behaviour the Accept test checks.
- Notes for later WPs:
  - `_selected_conflict()` is unchanged in shape (`_conflicts_cache[row]`); `_refresh_sync_status` now calls it both before rebuilding the table (to restore or clear selection) and the two conflict-action handlers already did.
  - `_insert_conflict` in `tests/test_sync_status_panel_ui.py` gained an optional `at=` parameter (default unchanged) so a test can control conflict sort order independently of insertion/id order; existing callers are unaffected.
  - Working tree: P3-7a and P3-8a are also uncommitted in this tree (P3-8a's dialog and test hunks are already in `1943770` alongside this WP's; P3-7a's are not). Whoever commits next should stage hunk by hunk where files are still shared, as the P3-7/P3-8 precedent above says.
