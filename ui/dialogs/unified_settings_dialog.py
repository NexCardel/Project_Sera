...
class UnifiedSettingsDialog(QDialog):
    ...
    def _build_page_general(self) -> QScrollArea:
        ...
        lay.addWidget(_sub_header("Automation Daemons"))

        self.fst_check = QCheckBox()
        lay.addWidget(_setting_row("FST \u2014 File Submission Tracker",
            "DOM mutation observer that captures filing confirmation messages from government web portals.",
            self.fst_check))

        self.sad_check = QCheckBox()
        lay.addWidget(_setting_row("SAD \u2014 Sera API Detection",
            "Passive network interceptor (fetch / XHR) for real-time JSON API capture from browser tabs.",
            self.sad_check))
    ...
    def _mcl_on_edit(self):
        ...
        dlg = ColumnEditDialog(self, cur["label"], cur.get("field_type", "text"),
                               cur.get("dropdown_options"), cur.get("is_identity", 0))
        if dlg.exec() == QDialog.Accepted:
