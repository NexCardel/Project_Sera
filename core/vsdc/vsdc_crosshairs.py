...
ITR_CROSSHAIRS: List[CrosshairDefinition] = [
    ...
    CrosshairDefinition(
        id="itr_login_auth",
        protocol="Income Tax",
        pattern=re.compile(r"[/#](?:login/password|login/otp|login/auth)(?:[?/#]|$)", re.IGNORECASE),
        target_crop="center_card",
        description="Taxpayer login step 2 (PAN & Secure Access Message identity seeding)",
        host_pattern=ITR_HOST_PATTERN,
    ...
    CrosshairDefinition(
        id="itr_logout",
        protocol="Income Tax",
        pattern=re.compile(r"(?:[/#](?:logout|sign-?out|session.?expire|session-?expired|session-?timeout)(?:[?/#]|$)|[/#]login(?:[?/#]|$))", re.IGNORECASE),
        target_crop="header",
