    ...
    window.addEventListener('SeraFSTApiCapture', (event) => {
        ...
        try {
            chrome.storage.local.get(['activeAutofillPayload', 'manualAssistPayload', 'mecpPayload', 'sadEnabled', 'trackerEnabled', 'allowedDomains'], (data) => {
                ...
                // Decouple from stale session storage: Only attach payload.client_id if PAN matches or payload has no conflicting identity
                ...
                if (!effectiveClientId && payload.client_id) {
                    ...
                    if (!capturedPan) {
                        // Wizard or status endpoint without PAN: allow fallback only if no distinct conflicting ARN
                        effectiveClientId = detail.arn ? null : payload.client_id;
                    } else if (payloadPan && capturedPan === payloadPan) {
                        // PANs match perfectly
                        effectiveClientId = payload.client_id;
                    } else {
                        // PAN belongs to a different client session (e.g. e-verifying different client) -> do NOT taint with stale client_id
